...
- 首次优化范围限定为 `skills/bamhub/maintenance/**/SKILL.md`。
...
5. 每次编辑后使用新的独立 agent 重跑对照并重新评分。新总分必须严格高于当前分数才可保留；否则使用 `git revert` 回滚该轮提交，禁止 `git reset --hard`。
...
- `eval_mode` 只能是 `full_test` 或 `dry_run`；不得把 dry run 描述为实测。
