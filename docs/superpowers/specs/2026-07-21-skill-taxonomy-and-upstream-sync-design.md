...
每次成功 `apply` 后，同步程序在每个受影响的目标根目录重建 `README.md`。内容至少包括：来源标识和仓库 URL、所跟踪的 ref、当前已接受提交、上次成功更新时间、如何使用 skill 的说明、可用 skill 清单及其 `SKILL.md` 元数据中的名称和简短描述，以及固定的通用执行流程。README 是当前 skill 集合的确定性使用指南，不保存文件变更列表。

调用期 AI 摘要的处理方式见 [2026-07-22-ephemeral-sync-summaries-design.md](2026-07-22-ephemeral-sync-summaries-design.md)：调用方可读取 `check` 的 JSON 报告并单独呈现摘要，但不会把摘要传给 `apply` 或写入 README、清单和提交。同步的正确性、提交更新和 PR 创建绝不依赖 AI。
...
- 每个受影响来源根目录生成 README，包含来源、提交、同步时间、使用说明、skill 清单和通用执行流程；
- 调用期 AI 摘要无法生成时不会阻断同步，README 始终是确定性使用指南。
