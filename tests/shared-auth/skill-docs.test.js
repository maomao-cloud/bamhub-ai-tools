...
test('skill docs keep auth runtime state repository-local', () => {
  const sharedAuth = readSkill('skills/shared-auth/SKILL.md');
  const kibanaSearch = readSkill('skills/kibana-search/SKILL.md');
...
test('shared-auth skill documents repository-local headless and import auth storage', () => {
  const content = readSkill('skills/shared-auth/SKILL.md');

  assert.match(content, /skills\/shared-auth\/\.local\/credentials\.json/);
...
test('kibana-search skill documents log query and shared-auth login commands', () => {
  const content = readSkill('skills/kibana-search/SKILL.md');

  assert.match(content, /bash skills\/kibana-search\/scripts\/kibana-search logs/);
  assert.match(content, /skills\/shared-auth\/scripts\/auth login/);
